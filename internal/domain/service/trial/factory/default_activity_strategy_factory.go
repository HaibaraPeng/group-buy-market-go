...
type DefaultActivityStrategyFactory struct {
	rootNode trial.StrategyHandler
}

// NewDefaultActivityStrategyFactory 创建默认活动策略工厂
func NewDefaultActivityStrategyFactory(rootNode trial.StrategyHandler) *DefaultActivityStrategyFactory {
...
func (f *DefaultActivityStrategyFactory) StrategyHandler() trial.StrategyHandler {
	return f.rootNode
}
